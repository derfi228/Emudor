...
opcodes = {
    ...
    0x00:'BRK', 0x08:'PHP', 0xCD:'CMP abs', 0xDD:'CMP abs,X',
    0x7E:'ROR abs,X', 0x1E:'ASL abs,X', 0x3E:'ROL abs,X', 0x5E:'LSR abs,X',
    0xDE:'DEC abs,X', 0xFE:'INC abs,X',
...
sizes = {'LDA #':2,'LDA zp':2,'LDA zp,X':2,'LDA abs':3,'LDA abs,X':3,'LDA abs,Y':3,
    ...
    'DEC abs,X':3,'INC abs,X':3,'ROR abs,X':3,'ASL abs,X':3,'ROL abs,X':3,'LSR abs,X':3,
    'STA (zp),Y':2,'STA (zp,X)':2,'LDA (zp,X)':2,'LDA (zp),Y':2,
    'BRK':2,'PHP':1,'???(72)':1,
}

def disasm(data, base_off, start_addr, base_rom_addr, count):
    pos = start_addr - base_rom_addr
    addr = start_addr
    printed = 0
    while printed < count and pos < 0x2000:
...
print('=== Bank 11 at $8000 (task runner) ===')
disasm(data, b11off, 0x8000, 0x8000, 40)

print('\n=== Bank 12 at $A79B ===')
disasm(data, b12off, 0xA79B, 0xA000, 40)

print('\n=== Bank 11 at $9E00 (searching for init code after dispatcher) ===')
disasm(data, b11off, 0x9E00, 0x8000, 30)

# Search for writes to $7CC0 in bank 11
print('\n=== Searching for STA abs,X $7CC0 (=$9D or $99) in bank 11 ===')
# $9D = STA abs,X, lo byte = $C0, hi byte = $7C
for i in range(0x1FFE):
    if data[b11off+i] == 0x9D and data[b11off+i+1] == 0xC0 and data[b11off+i+2] == 0x7C:
        print(f'  Found STA $7CC0,X at ${0x8000+i:04X}')
    if data[b11off+i] == 0x99 and data[b11off+i+1] == 0xC0 and data[b11off+i+2] == 0x7C:
        print(f'  Found STA $7CC0,Y at ${0x8000+i:04X}')
    if data[b11off+i] == 0x8D and data[b11off+i+1] == 0xC0 and data[b11off+i+2] == 0x7C:
        print(f'  Found STA $7CC0 at ${0x8000+i:04X}')

# Search in bank 12
print('\n=== Searching for STA $7CC0* in bank 12 ===')
for i in range(0x1FFE):
    if data[b12off+i] in (0x9D, 0x99, 0x8D) and data[b12off+i+1] == 0xC0 and data[b12off+i+2] == 0x7C:
        mn = {0x9D:'STA $7CC0,X', 0x99:'STA $7CC0,Y', 0x8D:'STA $7CC0'}[data[b12off+i]]
        print(f'  Found {mn} at ${0xA000+i:04X}')
