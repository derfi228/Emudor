...
- `tools/` — nestest.cpp (CPU regression), batman_trace.cpp, dump-скрипты на Python
...
3. Обнови статус issue на 'in_progress':
   ...
   import sqlite3
   c = sqlite3.connect('agent_data/issues.db')
   c.execute('UPDATE issues SET status=? WHERE id=?', ('in_progress', <ID>))
   c.commit(); c.close()
   "
   ```
...
9. Запусти конкретный test ROM из issue через headless:
   ```bash
   ./build/debug/emudor.exe --rom <ROM> --console <C> --frames 600 \
     --screenshot agent_data/screenshots/check.png --headless
   ```
...
10. Если PASSED и тесты не сломаны:
    ...
    - Обнови issue:
      ...
      import sqlite3
      c = sqlite3.connect('agent_data/issues.db')
      c.execute('UPDATE issues SET status=?, fixed_in_commit=? WHERE id=?',
                ('fixed', '<hash>', <ID>))
      c.commit(); c.close()
      "
      ```
...
- Если нужно архитектурное решение — создай запись:
  ...
  c.execute('INSERT INTO questions_for_user (agent, question, options) VALUES (?, ?, ?)',
            ('coder', 'Текст вопроса', json.dumps(['Вариант А', 'Вариант Б'])))
  c.commit(); c.close()
  "
  ```
  И остановись, не угадывай.
- После любого фикса прогоняй `./build/debug/nestest.exe roms/nes/nestest.nes roms/nes/nestest.log`
  (regression runner) и юнит-тесты — NES должен оставаться зелёным
