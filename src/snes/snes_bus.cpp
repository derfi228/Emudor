...
// Открытая шина держит ПОСЛЕДНЕЕ значение, прошедшее по шине — как при записи,
// так и при чтении (включая выборку опкодов и операндов). Раньше её обновляла
// только запись, и тогда «STA <пустой адрес>» с последующим чтением того же
// адреса возвращал записанное — появлялась фантомная память. Street Fighter II
// именно так и определяет наличие ОЗУ в картридже ($30:6000): тест ложно
// проходил, игра ставила флаг и переставала слушать START в демо-режиме.
uint8_t SnesBus::read(uint32_t addr)
{
    uint8_t v = readInternal(addr);
    openBus_ = v;
    return v;
}

uint8_t SnesBus::readInternal(uint32_t addr)
{
...
uint8_t SnesBus::readIO(uint16_t addr)
{
    ...
    // APU: $2140–$2143 (порты связи с SPC700).
    // Синхронность обеспечивает co-scheduler (SPC исполняется вперемежку с CPU),
    // поэтому прогон SPC здесь не нужен — просто читаем разделяемый порт.
    ...
    if (addr == 0x4212) {
        ...
        if (ppu_) {
            ...
            int d    = ppu_->curDot();
            int vbl  = ppu_->vblankStart();                     // 225 или 240 (overscan)
            if (line >= vbl)       v |= 0x80;                   // VBlank
            ...
            if (nmitimen_ & 0x01) {
                int t = (line - vbl) * 341 + d - 32;              // дотов от старта опроса
...
void SnesBus::writeIO(uint16_t addr, uint8_t data)
{
    ...
    // APU: $2140–$2143 (порты связи с SPC700).
    // Синхронность обеспечивает co-scheduler — просто пишем в разделяемый порт.
    if (apu_ && addr >= 0x2140 && addr <= 0x2143) {
